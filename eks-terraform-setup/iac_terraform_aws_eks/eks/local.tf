locals {
  ...
  worker_security_group_id = coalesce(join("", aws_security_group.workers_security_group.*.id), var.worker_security_group_id)
  default_iam_role_id      = element(concat(aws_iam_role.workers.*.id, list("")), 0)
  kubeconfig_name          = var.kubeconfig_name == "" ? "eks_${var.cluster_name}" : var.kubeconfig_name

  worker_nodes_group_defaults = {
    ...
    instance_type                 = "t3.micro"                 # Size of the workers instances.
    ...
    subnets                       = var.public_subnets                # A list of subnets to place the worker nodes in. i.e. ["subnet-123", "subnet-456", "subnet-789"]
  ...
  worker_nodes_on_demand_groups_defaults = merge(local.worker_nodes_group_defaults,var.worker_nodes_on_demand_groups)



  worker_nodes_mixed_groups_defaults = merge(var.worker_nodes_mixed_groups,local.worker_nodes_group_defaults)
